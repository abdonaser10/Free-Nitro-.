...
client.on('message', message => {
    if(message.content.startsWith(prefix + "toggleAntihack")) {
        ...
        if(!antihack[message.guild.id]) antihack[message.guild.id] = {
          ...
          if(antihack[message.guild.id].onoff === 'Off') return [message.channel.send(`**The AntiBots Is OFF!**`), antihack[message.guild.id].onoff = 'On']
          if(antihack[message.guild.id].onoff === 'On') return [message.channel.send(`**The AntiBots Is OFF!**`), antihack[message.guild.id].onoff = 'Off']
...
client.on("reachLimit", (limit)=> {
    ...
    log.send(`<@${limit.user.id}>`+"**\Try To Break Through (!)**");
    limit.guild.owner.send(limit.user.username+"**\Try To Break Through (!)**")
